...
def gene_rollup_highest_impact(initial_df, samples, cols):
    ...
    pivoted_df = pd.pivot_table(filtered_df, rows=["GENE_SYMBOL", "Sample"], cols=["HIGHEST_IMPACT"], values=["Sample_Data"], aggfunc=np.count_nonzero, fill_value=0)
...
def process_files(input, samples, delim):
