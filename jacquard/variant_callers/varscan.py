...
class Varscan():
    ...
    def _validate_vcf_fileset(self, vcf_readers):
        if len(vcf_readers) != 2:
            raise utils.JQException("VarScan directories should have exactly two input VCF files per patient, but found [{}].".format(len(vcf_readers)))
    ...
    def _validate_hc_fileset(self, hc_candidates):
        if len(hc_candidates) != 2:
            raise utils.JQException("VarScan directories should have exactly 2 input somatic HC files per patient, but found [{}].".format(len(hc_candidates)))
        ...
        for i,name in enumerate(tmp):
            if "indel" in name:
                tmp[i] = "indel"
        if not (tmp[0] == "snp" and tmp[1] == "indel") and not (tmp[1] == "snp" and tmp[0] == "indel"): 
            raise utils.JQException("Each patient in a VarScan directory should have a somatic HC snp file and indel file.")
