...
def validate_directories(input_file=None, output_dir=None):
    if input_file:
        ...
        try:
            ...
            logger.error("Specified input directory [{}] cannot be read. "+
                         "Check permissions and try again.", input_file)
            exit(1)
    if output_dir:
...
def validate_files(input_file=None, output_file=None):
    ...
    if output_file:
        if not os.path.isfile(output_file):
            logger.error("Given output parameter [{}] is not a valid file path. "+
                         "Check parameters and try again", output_file)
