#pylint: disable=line-too-long, maybe-no-member, unused-variable, no-member
#pylint: disable=unused-wildcard-import, unnecessary-lambda, wildcard-import
#pylint: disable=invalid-name, too-many-public-methods, redefined-builtin, bare-except
import ast
import pandas as pd
from pandas import *
import unittest
...
import os
from jacquard.pivot_variants import PivotError, VariantPivoter, expand_format, merge_samples, create_initial_df, project_prepivot, build_pivoter, append_to_annot_df, melt_samples, validate_parameters, validate_format_tags, rearrange_columns, change_order, determine_input_keys, get_headers_and_readers
...
2	43	C''')
    ...
    def test_expand_format(self):
        #pylint: disable=no-self-use, trailing-whitespace
        dataString = \
...
chr1	4	A	T'''
    ...
    def test_annot_files_equal(self):
        ...
        file_list = [script_dir + "/functional_tests/test_input/P2_test_input.txt", script_dir + "/functional_tests/test_input/P5_test_input.txt"]
        for file in file_list:
            df = pd.read_csv(file, sep="\t", header=1, dtype='str', index_col=False)
            annot_df = append_to_annot_df(df, annot_df)

        expected_df = pd.read_csv(script_dir + "/functional_tests/test_annotation/P2P5_combined_annotation.txt", sep="\t", header=False, dtype='str', index_col=False)
